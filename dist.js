...
InitState.recipeTable = null; // TODO RecipeTable type instead of any
// Contains collections of items and recipes. (solve.js)

InitState.solver = null; // TODO Solver type instead of any
// Contains module and factory settings, as well as other settings. (factory.js)

InitState.spec = null; // TODO FactorySpec type instead of any
// Map from module name to Module object.

InitState.modules = null; // TODO IObjectMap<Modules> type instead of any
// Array of modules, sorted by 'order'.

InitState.sortedModules = null; // Map from short module name to Module object.

InitState.shortModules = null; // TODO IObjectMap<Module> type instead of any
// Array of arrays of modules, separated by category and sorted.

InitState.moduleRows = null; // TODO Module[][] type instead of any
// Array of Belt objects, sorted by speed.

InitState.belts = null; // TODO Belt[] type instead of any
// Array of Fuel objects, sorted by value.

InitState.fuel = null; // TODO Fuel[] type instead of any
// Array of item groups, in turn divided into subgroups. For display purposes.

InitState.itemGroups = null; // TODO Item[][][] type instead of any
// Boolean with whether to use old (0.16) calculations.

InitState.useLegacyCalculations = false; // Size of the sprite sheet, as [x, y] array.

InitState.spriteSheetSize = null;
InitState.initDone = false;
exports.InitState = InitState; // Set the page back to a state immediately following initial setup, but before
...
function reset() {
  globals_1.window.location.hash = "";
...
exports.reset = reset; // TODO change callback type to (data: Data) => void instead of (data: any) => void

function loadDataRunner(modName, callback) {
  const xobj = new XMLHttpRequest();
  let mod = settings_1.SettingsState.MODIFICATIONS[modName];

  if (!mod) {
    mod = settings_1.SettingsState.MODIFICATIONS[settings_1.SettingsState.DEFAULT_MODIFICATION];
  }
...
exports.loadDataRunner = loadDataRunner;

function loadData(modName, settings) {
  InitState.recipeTable = new globals_1.window.RecipeTable($("#totals")[0]);
  ...
  loadDataRunner(modName, function (data) {
    globals_1.window.getSprites(data);
    const graph = globals_1.window.getRecipeGraph(data);
    InitState.modules = globals_1.window.getModules(data);
    InitState.sortedModules = globals_1.window.sorted(InitState.modules, m => InitState.modules[m].order);
    ...
    InitState.spec = new globals_1.window.FactorySpec(factories);
    ...
    InitState.belts = globals_1.window.getBelts(data);
    InitState.fuel = globals_1.window.getFuel(data, items).chemical;
    InitState.itemGroups = globals_1.window.getItemGroups(items, data);
    InitState.solver = new globals_1.window.Solver(items, recipes);
    ...
    if ("modules" in settings && settings.modules !== "") {
      ...
      for (let i = 0; i < moduleSettings.length; i++) {
        ...
        if (beaconSettings) {
          ...
          if (factory) {
            const count = globals_1.window.RationalFromFloat(Number(beaconSettingsSplit[1]));
    ...
    globals_1.window.location.hash = "#" + globals_1.window.formatSettings();
  });
}
...
function init() {
  const settings = globals_1.window.loadSettings(globals_1.window.location.hash);

  if (globals_1.window.OVERRIDE !== null) {
    settings_1.addOverrideOptions(globals_1.window.OVERRIDE);
  }
...
(() => {
  ...
  function moveObjToWindow(obj, key) {
    window[key] = obj[key];
  }
})();

},{"./globals":2,"./settings":4,"jquery":1}],4:[function(require,module,exports){
"use strict";
...
class Oil {
  constructor(recipeName, priorityName) {
...
SettingsState.DEFAULT_MODIFICATION = "0-16-51"; // Ideally we'd write this as a generalized function, but for now we can hard-
...
SettingsState.colorScheme = null; // TODO ColorScheme type instead of any type

SettingsState.seconds = globals_1.window.one;
SettingsState.minutes = globals_1.window.RationalFromFloat(60);
SettingsState.hours = globals_1.window.RationalFromFloat(3600);
...
SettingsState.preferredFuel = null; // TODO Fuel type instead of any type

SettingsState.OIL_OPTIONS = [new Oil("advanced-oil-processing", "default"), new Oil("basic-oil-processing", "basic"), new Oil("coal-liquefaction", "coal")];
...
SettingsState.preferredBeltSpeed = null; // TODO Rational type instead of any type

SettingsState.DEFAULT_PIPE = globals_1.window.RationalFromFloat(17);
SettingsState.minPipeLength = SettingsState.DEFAULT_PIPE;
SettingsState.maxPipeThroughput = null; // TODO Rational type instead of any type
...
function addOverrideOptions(version) {
  ...
  SettingsState.MODIFICATIONS[tag + "x"] = new Modification("Local game data " + version + " - Expensive", "local-" + version + "-expensive.json");
...
exports.renderDataSetOptions = renderDataSetOptions; // Returns currently-selected data set.

function currentMod() {
  const elem = document.getElementById("data_set");
  return elem.value;
}
...
function renderColorScheme(settings) {
  ...
  if (!colorSelector.hasChildNodes()) {
    for (let i = 0; i < globals_1.window.colorSchemes.length; i++) {
      const scheme = globals_1.window.colorSchemes[i];
      const option = document.createElement("option");
...
function setColorScheme(schemeName) {
  for (let i = 0; i < globals_1.window.colorSchemes.length; i++) {
    if (globals_1.window.colorSchemes[i].name === schemeName) {
      SettingsState.colorScheme = globals_1.window.colorSchemes[i];
...
function renderPrecisions(settings) {
  ...
  document.getElementById("rprec").value = String(SettingsState.ratePrecision);
  SettingsState.countPrecision = SettingsState.DEFAULT_COUNT_PRECISION;
  ...
  document.getElementById("fprec").value = String(SettingsState.countPrecision);
}

exports.renderPrecisions = renderPrecisions;

function renderMinimumAssembler(settings) {
  let min = SettingsState.DEFAULT_MINIMUM; // Backward compatibility.

  if ("use_3" in settings && settings.use_3 === "true") {
    min = "3";
  }

  const assemblers = init_1.InitState.spec.factories.crafting;
  ...
  const dropdown = globals_1.window.makeDropdown(globals_1.window.d3.select(node));
  const inputs = dropdown.selectAll("div").data(assemblers).join("div");
  const labels = globals_1.window.addInputs(inputs, "assembler_dropdown", (d, i) => String(i + 1) === min, (d, i) => globals_1.window.changeMin(String(i + 1)));
  labels.append(d => globals_1.window.getImage(d, false, dropdown.node()));
  cell.replaceChild(node, oldNode);
}
...
function setMinimumAssembler(min) {
  init_1.InitState.spec.setMinimum(min);
...
function renderFurnace(settings) {
  ...
  if (furnaceName !== init_1.InitState.spec.furnace.name) {
    init_1.InitState.spec.setFurnace(furnaceName);
  }
  ...
  const node = document.createElement("span");
  node.id = "furnace";
  const furnaces = init_1.InitState.spec.factories.smelting;
  const dropdown = globals_1.window.makeDropdown(globals_1.window.d3.select(node));
  const inputs = dropdown.selectAll("div").data(furnaces).join("div");
  const labels = globals_1.window.addInputs(inputs, "furnace_dropdown", d => d.name === furnaceName, globals_1.window.changeFurnace);
  labels.append(d => globals_1.window.getImage(d, false, dropdown.node()));
  cell.replaceChild(node, oldNode);
}
...
function renderFuel(settings) {
  ...
  node.id = "fuel";
  const dropdown = globals_1.window.makeDropdown(globals_1.window.d3.select(node));
  const inputs = dropdown.selectAll("div").data(init_1.InitState.fuel).join("div");
  const labels = globals_1.window.addInputs(inputs, "fuel_dropdown", d => d.name === fuelName, globals_1.window.changeFuel);
  labels.append(d => {
    const im = globals_1.window.getImage(d, false, dropdown.node());
...
function setPreferredFuel(name) {
  for (let i = 0; i < init_1.InitState.fuel.length; i++) {
    const f = init_1.InitState.fuel[i];
...
function renderOil(settings) {
  let oil = SettingsState.DEFAULT_OIL; // Named "p" for historical reasons.
  ...
  const labels = globals_1.window.addInputs(inputs, "oil_dropdown", d => d.priority === oil, globals_1.window.changeOil);
  labels.append(d => globals_1.window.getImage(init_1.InitState.solver.recipes[d.name], false, dropdown.node()));
  cell.replaceChild(node, oldNode);
}
...
function setOilRecipe(name) {
  init_1.InitState.solver.removeDisabledRecipes(SettingsState.OIL_EXCLUSION[SettingsState.oilGroup]);
  SettingsState.oilGroup = name;
  init_1.InitState.solver.addDisabledRecipes(SettingsState.OIL_EXCLUSION[SettingsState.oilGroup]);
}
...
function renderKovarex(settings) {
  ...
  setKovarex(k);
  const input = document.getElementById("kovarex");
  input.checked = k;
}
...
function setKovarex(enabled) {
  ...
  if (enabled) {
    init_1.InitState.solver.removeDisabledRecipes({
      "kovarex-enrichment-process": true
    });
  } else {
    init_1.InitState.solver.addDisabledRecipes({
      "kovarex-enrichment-process": true
    });
  }
}
...
function renderBelt(settings) {
  ...
  node.id = "belt";
  const dropdown = globals_1.window.makeDropdown(globals_1.window.d3.select(node));
  const inputs = dropdown.selectAll("div").data(init_1.InitState.belts).join("div");
  const labels = globals_1.window.addInputs(inputs, "belt_dropdown", d => d.name === SettingsState.preferredBelt, globals_1.window.changeBelt);
  labels.append(d => globals_1.window.getImage(new globals_1.window.BeltIcon(init_1.InitState.solver.items[d.name], d.speed), false, dropdown.node()));
  cell.replaceChild(node, oldNode);
}
...
function setPreferredBelt(name) {
  for (let i = 0; i < init_1.InitState.belts.length; i++) {
    const belt = init_1.InitState.belts[i];
...
function renderPipe(settings) {
  ...
  setMinPipe(pipe);
  document.getElementById("pipe_length").value = SettingsState.minPipeLength.toDecimal(0);
}

exports.renderPipe = renderPipe;

function setMinPipe(lengthString) {
  SettingsState.minPipeLength = globals_1.window.RationalFromString(lengthString);
  SettingsState.maxPipeThroughput = globals_1.window.pipeThroughput(SettingsState.minPipeLength);
}
...
function renderMiningProd(settings) {
  ...
  init_1.InitState.spec.miningProd = getMprod();
}
...
function getMprod() {
  ...
  return globals_1.window.RationalFromFloats(Number(mprod), 100);
}

exports.getMprod = getMprod; // default module

function renderDefaultModule(settings) {
  let defaultModule = null;

  if ("dm" in settings) {
    defaultModule = init_1.InitState.shortModules[settings.dm];
  }

  init_1.InitState.spec.setDefaultModule(defaultModule);
  const oldDefMod = document.getElementById("default_module");
  const cell = oldDefMod.parentNode;
  const node = document.createElement("span");
  node.id = "default_module";
  globals_1.window.moduleDropdown(globals_1.window.d3.select(node), "default_module_dropdown", d => d === defaultModule, globals_1.window.changeDefaultModule);
...
function renderDefaultBeacon(settings) {
  ...
  let defaultCount = globals_1.window.zero;

  if ("db" in settings) {
    defaultBeacon = init_1.InitState.shortModules[settings.db];
  }

  if ("dbc" in settings) {
    defaultCount = globals_1.window.RationalFromString(settings.dbc);
  }

  init_1.InitState.spec.setDefaultBeacon(defaultBeacon, defaultCount);
  ...
  globals_1.window.moduleDropdown(globals_1.window.d3.select(node), "default_beacon_dropdown", d => d === defaultBeacon, globals_1.window.changeDefaultBeacon, d => d === null || d.canBeacon());
  cell.replaceChild(node, oldDefMod);
}
...
function renderVisualizerType(settings) {
  ...
  const input = document.getElementById("vis_" + SettingsState.visualizer);
  input.checked = true;
}
...
function renderVisualizerDirection(settings) {
  ...
  const input = document.getElementById("visdir_" + SettingsState.visDirection);
  input.checked = true;
}
...
function renderValueFormat(settings) {
  ...
  const input = document.getElementById(SettingsState.displayFormat + "_format");
  input.checked = true;
}
